...
function Dashboard() {
  ...
  const fetchDashboard = async () => {
    // 1) Load the owner's restaurant.
    // A 404 simply means the owner has not registered a
    // restaurant yet, so handle it gracefully (no error).
    ...
    try {
      ...
      if (response.data.success) {
        loadedRestaurant = response.data.restaurant;
      }
    } catch (error) {
    ...
    // 2) Orders can only exist once the restaurant is Approved.
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FFFCF7]">
        ...
        <div className="text-center space-y-3">
          ...
          <p className="text-[#8A7461] font-medium">Loading dashboard...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#FFFCF7]">
      <style>{`
        ...
        .count-up { animation: countUp 0.4s cubic-bezier(0.22, 1, 0.36, 1) both; }
        .badge-sizzle { animation: badgeSizzle 3s ease-in-out infinite; }
        .stat-card, .action-card { transition: transform 0.25s ease, box-shadow 0.25s ease, border-color 0.25s ease; }
        .stat-card:hover { transform: translateY(-3px); box-shadow: 0 10px 28px rgba(217,52,37,0.08); }
        .action-card:hover { transform: translateY(-4px); box-shadow: 0 12px 30px rgba(217,52,37,0.1); border-color: #F0D9A8; }
      `}</style>
        ...
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#8A7461] font-medium">Restaurant Panel</p>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-10">
        ...
        {!restaurant ? (
          <div className="mb-8 rounded-2xl border border-[#F0DFB0] bg-[#FDF3E0] p-6 fade-up">
            ...
            <p className="mt-2 text-[#8A6412]">
              Food management becomes available after an admin approves your registration.
            </p>
            <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {/* Total Orders */}
          <div className="stat-card count-up bg-white rounded-2xl border border-[#F0E4D4] shadow-sm p-6" style={{ animationDelay: "0.05s" }}>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[#8A7461] text-sm font-medium">Total Orders</p>
                <p className="text-3xl font-extrabold text-[#241608] mt-2" style={{ fontFamily: "'Baloo 2', sans-serif" }}>
                  {orders.length}
                </p>
              </div>
          ...
          {/* Pending */}
          <div className="stat-card count-up bg-white rounded-2xl border border-[#F0E4D4] shadow-sm p-6" style={{ animationDelay: "0.1s" }}>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[#8A7461] text-sm font-medium">Active Orders</p>
                <p className="text-3xl font-extrabold text-[#D93425] mt-2" style={{ fontFamily: "'Baloo 2', sans-serif" }}>
          ...
          {/* Delivered */}
          <div className="stat-card count-up bg-white rounded-2xl border border-[#F0E4D4] shadow-sm p-6" style={{ animationDelay: "0.15s" }}>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[#8A7461] text-sm font-medium">Delivered</p>
                <p className="text-3xl font-extrabold text-[#4A7A2E] mt-2" style={{ fontFamily: "'Baloo 2', sans-serif" }}>
          ...
          {/* Sales */}
          <div className="stat-card count-up bg-white rounded-2xl border border-[#F0E4D4] shadow-sm p-6" style={{ animationDelay: "0.2s" }}>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[#8A7461] text-sm font-medium">Paid Sales</p>
                <p className="text-3xl font-extrabold text-[#1D5FA8] mt-2" style={{ fontFamily: "'Baloo 2', sans-serif" }}>
                  ₹{totalSales}
                </p>
              </div>
        ...
        <div className="mt-10 fade-up" style={{ animationDelay: "0.1s" }}>
          <h3 className="text-xl font-bold text-[#241608] mb-5" style={{ fontFamily: "'Baloo 2', sans-serif" }}>
            Quick Actions
          </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            >
              ...
              <h4 className="text-lg font-bold text-[#241608]" style={{ fontFamily: "'Baloo 2', sans-serif" }}>
                Manage Menu
              </h4>
              <p className="text-[#8A7461] text-sm mt-1">
                Add, edit and remove food items.
              </p>
              <p className="text-[#D93425] font-bold mt-4">Open Menu →</p>
            </button>
            ...
            >
              ...
              <p className="text-[#D93425] font-bold mt-4">View Orders →</p>
            </button>
            ...
            >
              ...
              <p className="text-[#D93425] font-bold mt-4">Add Food →</p>
            </button>
          </div>
        </div>
        ...
        <div className="mt-10 fade-up" style={{ animationDelay: "0.15s" }}>
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-[#8A7461] text-sm mt-1">Latest customer orders.</p>
            </div>
          ...
          {orders.length === 0 ? (
            <div className="bg-white rounded-2xl border border-[#F0E4D4] p-10 text-center">
              ...
              <p className="font-bold text-[#241608]">No orders yet</p>
              <p className="text-[#8A7461] text-sm mt-1">
            ...
            <div className="bg-white rounded-2xl border border-[#F0E4D4] shadow-sm overflow-hidden">
              <div className="divide-y divide-[#F0E4D4]">
                {orders.slice(0, 5).map((order) => (
                  ...
                  >
                    <div>
                      <p className="font-bold text-[#241608]">#{order._id}</p>
